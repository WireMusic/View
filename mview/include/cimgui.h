...
typedef struct ImVector_ImGuiPlatformMonitor 
{
    ...
    inline void reserve(int new_capacity) 
    {
        if (new_capacity <= Capacity)
            return; 
        ImGuiPlatformMonitor* new_data = (ImGuiPlatformMonitor*)IM_ALLOC((size_t)new_capacity * sizeof(ImGuiPlatformMonitor)); 
        if (Data) 
        {
            memcpy(new_data, Data, (size_t)Size * sizeof(ImGuiPlatformMonitor)); 
            IM_FREE(Data);
        }
        Data = new_data;
    ...
    inline void push_back(const ImGuiPlatformMonitor& v) { if (Size == Capacity) reserve(_grow_capacity(Size + 1)); memcpy(&Data[Size], &v, sizeof(v)); Size++; }
